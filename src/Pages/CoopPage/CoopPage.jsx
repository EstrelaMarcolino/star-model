...
export default function CoopPage() {
    ...
    useEffect(() => {
        const getCoop = async () => {
            const res = await axios.get(`${ServerURL}/agencia/${path}`)
            setCoop(res.data)
        }
        getCoop()
    }, [path])
    ...
    const matchFunction = async () => {
        try {
            const fiMatch = await axios.get(`${ServerURL}/chat/match/${path}`, {
                headers: { userid: user._id }
            })

            // console.log(fiMatch.data._id)
            window.location.replace(`/chat/${fiMatch.data._id}`);
    ...
    return (
        <div className='fullConteinerCoopPage'>
            <Header />
            <Menu />
            {/* <MenuLeft grup={user?.nome} room={user._id} /> */}
            <div className="conteinerCoop">
                <div className="sideBarCoop">
                    <div className="descCoop">
                        ...
                        {user.profilePic && (
                            <div className="avaliacoesCoop">
                                <div className="chatButton" onClick={matchFunction}>Chat</div>
                            </div>
                        )}
                    </div>
                    {user._id === path && (
